...
type Article struct {
	// 可以给字段指定特定规则  gorm：数据库数据规则  绑定json格式是为了前后端交互准备
	Category Category `gorm:"foreignkey:Cid"` // 指定外键
	gorm.Model
...
func GetCateArt(id int, pageSize int, pageNum int) ([]Article, int, int64) {
	...
	// 预加载 db.Preload("Category") 主结构体的字段名  Find(&cateArtList) 内联的数据库表名
	// 预加载内联的数据库
	err = db.Preload("Category").Limit(pageSize).Offset((pageNum-1)*pageSize).Where(
		"cid =?", id).Find(&cateArtList).Error
	// db.Model(&cateArtList) 指定您要运行数据库操作的模型---cateArtList
...
func GetArtInfo(id int) (Article, int) {
	...
	// db.Model(&art) 指定您要运行数据库操作的模型---art
...
func GetArt(pageSize int, pageNum int) ([]Article, int, int64) {
	...
	// 单独计数
	// db.Model(&articleList) 指定您要运行数据库操作的模型---articleList
